...
4. **Contrôles** du type : `min()`, `max()`, `pattern()`, format de date, d'email...
5. **`assert()`** : tes règles.
...
Schema::mixed()->before(fn($v) => preg_replace('/\D/', '', (string) $v))->pattern('\d{10}');
```
...
- une chaîne : `min()`/`max()` comptent les **caractères** (`mb_strlen`), pas les octets ;
- un nombre : `min()`/`max()` comparent la valeur, bornes incluses ;
- `pattern()` : une expression régulière sans délimiteurs, qui doit correspondre à **toute** la chaîne (`'\d{5}'` refuse `'750011'`). Unicode activé.

Codes : `min`, `max`, `pattern`.
...
- Une structure absente (`null`, `''`, `[]`) est traitée comme `[]` : ses défauts et ses `required()` s'appliquent quand même. `required()` et `default()` n'ont donc pas d'effet sur une structure.
...
- `min()` et `max()` comptent les éléments. Codes : `min`, `max`.
...
Si aucun schéma ne valide la valeur, `errors()` contient les erreurs du schéma qui en a **le moins** (le premier en cas d'égalité). Avec un champ `type` qui départage les variantes, ce sont donc les erreurs de la bonne variante : `['type' => 'paypal', 'email' => 'bad']` donne `['email' => 'email']`.
...
- Un paramètre avec une valeur par défaut prend ce défaut.
- Un paramètre sans défaut et non nullable devient `required()`, sauf les tableaux et les classes.
...
Quand le nettoyage d'un champ échoue (code `type`), `values()` contient sa valeur par défaut plutôt que la saisie.
...
| `pattern` | `pattern()` |
| `date`, `datetime`, `time`, `email`, `url` | format de `Schema::date()`... |
| `anyOf` | aucune valeur de `anyOf()` ou `enum()` ne correspond |
...
Les filtres (`Bredala\Validation\Filters`) prennent une valeur et en rendent une autre ; les types s'en servent pour nettoyer l'entrée. Ils restent publics, pour s'en servir dans un `before()` ou un `transform()`. Elles lèvent une `ValidationException` (code `type`) quand la valeur est inconvertible.
...
La chaîne de traitement de `input()` et `text()` : réparation UTF-8 (`htmlentities` puis `html_entity_decode`) → retrait des balises → suppression des caractères non imprimables et de `U+FFFD` → normalisation des espaces exotiques → fusion des espaces consécutives → rognage → `''` devient `null`.

Quelques conséquences :

- Une saisie composée uniquement d'espaces devient `null`.
- Les valeurs numériques court-circuitent toute la chaîne : `42` devient `'42'` sans traitement.
- Une valeur non chaîne et non numérique lève `type` : tableaux, objets, et notamment `true`.
...
- `BooleanFilter::sanitize(mixed $value): ?bool` Les booléens passent ; `0`/`1` (numérique ou chaîne) donnent faux/vrai ; les autres chaînes passent par `FILTER_VALIDATE_BOOLEAN` (`yes`/`no`, `on`/`off`, `true`/`false`) ; le reste lève `type`.
...
Les règles (`Bredala\Validation\Rules`) prennent une valeur et rendent un booléen, pour s'en servir dans un `assert()` ou un `format()`.

- `StringRule::isDate(string $value, string ...$formats): bool` Le contrôle strict des dates, utilisable seul.
...
| `StringField::required($v)` | `->required()` |
